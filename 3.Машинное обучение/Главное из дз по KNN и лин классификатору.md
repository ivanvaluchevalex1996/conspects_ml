...
```text
Экзамен: по домашке с ответами лучше всех тот, кто ЗАЗУБРИЛ ответы.
В ML так же: train-ошибка монотонно улучшается с ростом сложности модели.
```

| Модель | Ручка сложности | Что покажет train |
|:---|:---|:---|
| KNN | $k$ | при $k=1$ ошибка на train **ровно 0** |
| Полином | степень $d$ | при $d=300$ ошибка на train $\approx 0$ |
| L2-регуляризация | `reg` | чем меньше `reg`, тем меньше ошибка **на данных** (полный loss включает ещё и штраф $\text{reg}\sum W^2$, поэтому его числа между разными `reg` напрямую не сравнимы) |

Значит, критерий по train **всегда указывает на край диапазона** — на самую сложную модель. Внутреннего минимума у него нет, он не умеет сказать «стоп»:

```text
              ошибка на train    ошибка на val
мало учил         высокая           высокая     ← недообучение
нормально         низкая            низкая      ← вот сюда хотим
зазубрил         ~НОЛЬ             высокая     ← переобучение
```

> **Важно:** смотреть на train **можно** (разрыв train vs val — признак переобучения). **Выбирать** гиперпараметры по train — нельзя.

> Метрика на train смещена вниз по построению: мы сами её оптимизировали. Поэтому она мерит не «как модель обобщает», а «как она запомнила».
